...
页面按铂金、钻石、黄金、白银分组，同档按名单中的顺序展示。没有赞助商的档位仍保留标题与分区，名单位置留空。等级与顺序由主办方确认，不按累计金额自动调整。当前两位铂金赞助商依次为 DR.Yang、WoShiLaoCaiNiao，钻石档位空缺。

## 到账流水

`sponsorship-ledger.json` 保存完整到账记录，每笔到账单独追加，不需要流水编号。顶层 `currency` 统一为 `CNY`，`transactions` 中各笔包含：
